...
from deepmed_ctrain_utils.ner_metric import metric_for_ner
...
def convert_text_labels_to_ner_labels(text, labels, tokenizer):
...
def get_dataset_dict(tokenizer, label2id, use_cache=True):
    ...
    eval_dataset = eval_dataset.map(tokenize_and_align_labels, batched=True, writer_batch_size=10)
...
def compute_metrics_for_ner(eval_pred, id2label, entity_type):
    """
    ...
    results = metric_for_ner(y_true=y_true, y_pred=y_pred, entity_type=entity_type)
...
class ArmsNer:
    ...
    def test_train(self):
        ...
        trainer = Trainer(
            ...
            eval_dataset=self.dataset_dict['eval'].select(range(32)),
    ...
    def test(self):
        ...
        classifier = pipeline("ner",
                              model="/home/zyl/disk/PharmAI/Pipelines/components/deepmed_ctrain/output/arms_ner/models/run-wevaz2f2/checkpoint-492"
                              )
        print(1)
...
if __name__ == '__main__':
    ...
    # ArmsNer(gpus=gpus).evaluate()
    # d = ArmsNer(gpus=gpus)
    print(1)
    # ArmsNer(gpus=gpus).train_with_hp(n_trials=10)
    # ArmsNer(gpus=gpus).test()
